...
public class AscendingInvoke {
    ...
    public static void start(final Task task) {
        ...
        try {
            ...
            long curDelayHour = TimeUnit.MILLISECONDS.toHours(curDelay);
            if (level == MAX_LEVEL) {
                log.error("时间递增回调任务已经达到最大时间间隔{}小时、最大尝试次数{}次，放弃该条回调任务，任务数据={}",
                        curDelayHour, MAX_LEVEL, task.desc(), e);
                THREAD_LEVEL.remove();
                THREAD_DELAY.remove();
                return;
            }
            log.warn("回调失败，增加时间间隔，当前时间间隔{}小时，增加后时间间隔{}小时，当前第{}次尝试，一共会尝试{}次，"
                    + "任务数据={}", curDelayHour, curDelayHour << 1, level, MAX_LEVEL, task.desc(), e);
